 ...
 * Configuration:   Default configuration + ENTRY_D_MINUS_PULLUP_ACTIVATED + FAST_EXIT_NO_USB_MS=300
 *       USB D- :   PB3
 *       USB D+ :   PB4
 *       Entry  :   Power on
 *       LED    :   None
 ...
 *                      Activate the bootloader only if the D- pin is high, i.e. a pullup resistor
 *                      is attached and powered. Useful if the pullup is powered by USB V+
 *                      and NOT ATtiny VCC to save power. In this case often a schottky diode
 *                      is connected between V* and VCC.
 *                      The ENTRY_POWER_ON condition is ANDed to use this as the default configuration.
 *
 */
...
#define ENTRY_D_MINUS_PULLUP_ACTIVATED 6

#define ENTRYMODE ENTRY_D_MINUS_PULLUP_ACTIVATED
...
#elif ENTRYMODE==ENTRY_D_MINUS_PULLUP_ACTIVATED
  #define bootLoaderInit()
// We must reset PORF flag after checking for entry condition to prepare for the next time.
// To be able to interpret MCUSR flags in user program, it is copied to the OCR1C register.
// Use "if (MCUSR != 0) tMCUSRStored = MCUSR; else tMCUSRStored = GPIOR0;"
