...
int main(void) {
  ...
  osccal_default = OSCCAL;
    ...
    if (bootLoaderStartCondition() || (pgm_read_byte(BOOTLOADER_ADDRESS - TINYVECTOR_RESET_OFFSET + 1) == 0xff)) {
        ...
        uint8_t resetDetected = 0; // Flag to call OSCCAL calibration function or reset idlePolls directly after host reset ends.
#endif
...
#  if (OSCCAL_HAVE_XTAL == 0) || (FAST_EXIT_NO_USB_MS > 0)
                    /*
                     * Call OSCCAL calibration function or reset idlePolls only if USB is attached and after a reset, otherwise just skip it and wait for timeout.
...
#  if (OSCCAL_HAVE_XTAL == 0) || (FAST_EXIT_NO_USB_MS > 0)
                    resetDetected = 1;  // Set flag to wait for reset to end before calling OSCCAL calibration function or reset idlePolls.
