// Upgrade is an in-place firmware upgrader for tiny85 ad other AVR chips
// It takes the content of main.hex and stores it at the end of flash program space

// The magic happens in the Makefile, just do: "make clean; make"
//
// It does this step by step:
// - Compile main.c into main.hex
// - Convert main.hex into bootloader.raw
// - Convert bootloader.raw into bootloader.o and rename some symbols:
//   bootloader, bootloader_end, bootloader_size_sym
// - Build bootloader.h
//   ... that declares these values as external
//   ... and defines bootloader_address from the value BOOTLOADER_ADDRESS in Makefile.inc
// - Compile the file upgrade.c (this file) into upgrade.o
// - Link upgrade.o and bootloader.o into upgrade.hex

// To update the bootloader just uploade upgrade.hex via bootloader or ISP.
// It will firstly rewrite the interrupt vector table to disable the bootloader,
...
// bootloader_address, effectively bridging the interrupts in to the new bootloader's
// interrupts table.
//
...
// If you connect a piezo between pb1 and gnd you'll hear a bleep when the update
// is complete. You can also connect an LED with pb1 positive and gnd negative and
// it will blink
...
// this file is generated by make script, it declares external symbols from bootloader.o
